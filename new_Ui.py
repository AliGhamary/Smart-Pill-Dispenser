from tkinter import *
from tkinter.ttk import Progressbar
from tkinter.ttk import Combobox
from tkinter.ttk import Notebook
import tkinter.font
import sys, os
from xml.dom.expatbuilder import parseFragmentString
from time import sleep

#import RPi.GPIO as GPIO

class onKeyboard():
  def __init__(self,text_label=None,exp=""):
    self.text_box_label=text_label
    self.exp=exp

  def press(self,num):
    self.exp = self.exp + str(num)
    print(self.exp)
    text_box_label.delete(0,END)
    text_box_label.insert(0,self.exp)

  def Backspace(self):
    self.exp = self.exp[:-1]
    print(self.exp)
    text_box_label.delete(0,END)
    text_box_label.insert(0,self.exp)

  def Clear(self):
     self.exp=""
     text_box_label.delete(0,END)
  def Shift(self,key):
    global is_shift
    is_shift = not is_shift
    self.display(key)

  def display(self,key):
    if (True):
        # Adding keys line wise
        # First Line Button
        # tilda = Button(key, text='~', width=6, command=lambda:  self.press('~'))
        # tilda.place(x = 2, y = 280, width = 55, height = 55)

        num1 = Button(key, text='!', width=6, command=lambda:  self.press('!'))
        num1.place(x = 60, y = 280, width = 55, height = 55)

        num2 = Button(key, text='@', width=6, command=lambda:  self.press('@'))
        num2.place(x = 120, y = 280, width = 55, height = 55)

        num3 = Button(key, text='#', width=6, command=lambda:  self.press('#'))
        num3.place(x = 180, y = 280, width = 55, height = 55)

        num4 = Button(key, text='$', width=6, command=lambda:  self.press('$'))
        num4.place(x = 240, y = 280, width = 55, height = 55)

        num5 = Button(key, text='%', width=6, command=lambda:  self.press('%'))
        num5.place(x = 300, y = 280, width = 55, height = 55)

        num6 = Button(key, text='^', width=6, command=lambda:  self.press('^'))
        num6.place(x = 360, y = 280, width = 55, height = 55)

        num7 = Button(key, text='&', width=6, command=lambda:  self.press('&'))
        num7.place(x = 420, y = 280, width = 55, height = 55)

        num8 = Button(key, text='*', width=6, command=lambda:  self.press('*'))
        num8.place(x = 480, y = 280, width = 55, height = 55)

        num9 = Button(key, text='(', width=6, command=lambda:  self.press('('))
        num9.place(x = 540, y = 280, width = 55, height = 55)

        num0 = Button(key, text=')', width=6, command=lambda:  self.press(')'))
        num0.place(x = 600, y = 280, width = 55, height = 55)

        under = Button(key, text='_', width=6, command=lambda:  self.press('_'))
        under.place(x = 660, y = 280, width = 55, height = 55)

        plus = Button(key, text='+', width=6, command=lambda:  self.press('+'))
        plus.place(x = 720, y = 280, width = 55, height = 55)

        backspace = Button(
            key, text='BackSpace', font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7),width=6, command=self.Backspace)
        backspace.place(x = 780, y = 280, width = 55, height = 55)

        num_1 =  Button(key, text='1', width=6, command=lambda:  self.press('1'))
        num_1.place(x = 855, y = 280, width = 50, height = 50)

        num_2 =  Button(key, text='2', width=6, command=lambda:  self.press('2'))
        num_2.place(x = 913, y = 280, width = 50, height = 50)

        num_3 =  Button(key, text='3', width=6, command=lambda:  self.press('3'))
        num_3.place(x = 970, y = 280, width = 50, height = 50)
        # Second Line Buttons

        # tab_button = Button(key, text='Tab',font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), width=6,
        #                         command=lambda:  self.press('\t'))
        # tab_button.place(x = 2, y = 340, width = 55, height = 55)

        Q = Button(key, text='Q', width=6, command=lambda:  self.press('Q'))
        Q.place(x = 60, y = 340, width = 55, height = 55)

        W = Button(key, text='W', width=6, command=lambda:  self.press('W'))
        W.place(x = 120, y = 340, width = 55, height = 55)

        E = Button(key, text='E', width=6, command=lambda:  self.press('E'))
        E.place(x = 180, y = 340, width = 55, height = 55)
        R = Button(key, text='R', width=6, command=lambda:  self.press('R'))
        R.place(x = 240, y = 340, width = 55, height = 55)

        T = Button(key, text='T', width=6, command=lambda:  self.press('T'))
        T.place(x = 300, y = 340, width = 55, height = 55)

        Y = Button(key, text='Y', width=6, command=lambda:  self.press('Y'))
        Y.place(x = 360, y = 340, width = 55, height = 55)

        U = Button(key, text='U', width=6, command=lambda:  self.press('U'))
        U.place(x = 420, y = 340, width = 55, height = 55)

        I = Button(key, text='I', width=6, command=lambda:  self.press('I'))
        I.place(x = 480, y = 340, width = 55, height = 55)

        O = Button(key, text='O', width=6, command=lambda:  self.press('O'))
        O.place(x = 540, y = 340, width = 55, height = 55)

        P = Button(key, text='P', width=6, command=lambda:  self.press('P'))
        P.place(x = 600, y = 340, width = 55, height = 55)

        curly_l = Button(
            key, text='{', width=6, command=lambda:  self.press('{'))
        curly_l.place(x = 660, y = 340, width = 55, height = 55)

        curly_r = Button(key, text='}', width=6,
                             command=lambda:  self.press('}'))
        curly_r.place(x = 720, y = 340, width = 55, height = 55)
        
        back_slash = Button(key, text='\\', width=6,
                             command=lambda:  self.press('\\'))
        back_slash.place(x = 780, y = 340, width = 55, height = 55)
        
        num_4 =  Button(key, text='4', width=6, command=lambda:  self.press('4'))
        num_4.place(x =855, y = 340, width = 50, height = 50)

        num_5 =  Button(key, text='5', width=6, command=lambda:  self.press('5'))
        num_5.place(x = 913, y = 340, width = 50, height = 50)

        num_6 =  Button(key, text='6', width=6, command=lambda:  self.press('6'))
        num_6.place(x = 970, y = 340, width = 50, height = 50)
        # Third Line Buttons

        A = Button(key, text='A', width=6, command=lambda:  self.press('A'))
        A.place(x = 60, y = 400, width = 55, height = 55)

        S = Button(key, text='S', width=6, command=lambda:  self.press('S'))
        S.place(x = 120, y = 400, width = 55, height = 55)

        D = Button(key, text='D', width=6, command=lambda:  self.press('D'))
        D.place(x = 180, y = 400, width = 55, height = 55)

        F = Button(key, text='F', width=6, command=lambda:  self.press('F'))
        F.place(x = 240, y = 400, width = 55, height = 55)

        G = Button(key, text='G', width=6, command=lambda:  self.press('G'))
        G.place(x = 300, y = 400, width = 55, height = 55)

        H = Button(key, text='H', width=6, command=lambda:  self.press('H'))
        H.place(x = 360, y = 400, width = 55, height = 55)

        J = Button(key, text='J', width=6, command=lambda:  self.press('J'))
        J.place(x = 420, y = 400, width = 55, height = 55)

        K = Button(key, text='K', width=6, command=lambda:  self.press('K'))
        K.place(x = 480, y = 400, width = 55, height = 55)

        L = Button(key, text='L', width=6, command=lambda:  self.press('L'))
        L.place(x = 540, y = 400, width = 55, height = 55)

        colon = Button(key, text=':', width=6,
                           command=lambda:  self.press(':'))
        colon.place(x = 600, y = 400, width = 55, height = 55)

        quotation = Button(key, text='"', width=6,
                               command=lambda:  self.press('"'))
        quotation.place(x = 660, y = 400, width = 55, height = 55)

        pipe =  Button(key, text='|', width=6, command=lambda:  self.press('|'))
        pipe.place(x = 720, y = 400, width = 55, height = 55)

        # enter =  Button(key, text='Enter', width=6,
        #                    command=lambda:  self.press('\n'))
        # enter.place(x = 780, y = 400, width = 55, height = 55)

        num7 =  Button(key, text='7', width=6, command=lambda:  self.press('7'))
        num7.place(x = 855, y = 400, width = 50, height = 50)

        num8 =  Button(key, text='8', width=6, command=lambda:  self.press('8'))
        num8.place(x = 913, y = 400, width = 50, height = 50)


        num9 =  Button(key, text='9', width=6, command=lambda:  self.press('9'))
        num9.place(x = 970, y = 400, width = 50, height = 50)


        # Fourth line Buttons

        # shift =  Button(key, text='Shift', width=6, command=lambda: self.Shift(key))
        # shift.place(x = 2, y = 460, width = 55, height = 55)

        Z =  Button(key, text='Z', width=6, command=lambda:  self.press('Z'))
        Z.place(x = 60, y = 460, width = 55, height = 55)
        X =  Button(key, text='X', width=6, command=lambda:  self.press('X'))
        X.place(x = 120, y = 460, width = 55, height = 55)

        C =  Button(key, text='C', width=6, command=lambda:  self.press('C'))
        C.place(x = 180, y = 460, width = 55, height = 55)

        V =  Button(key, text='V', width=6, command=lambda:  self.press('V'))
        V.place(x = 240, y = 460, width = 55, height = 55)

        B =  Button(key, text='B', width=6, command=lambda:  self.press('B'))
        B.place(x = 300, y = 460, width = 55, height = 55)

        N =  Button(key, text='N', width=6, command=lambda:  self.press('N'))
        N.place(x = 360, y = 460, width = 55, height = 55)

        M =  Button(key, text='M', width=6, command=lambda:  self.press('M'))
        M.place(x = 420, y = 460, width = 55, height = 55)

        ang_l =  Button(key, text='<', width=6, command=lambda:  self.press('<'))
        ang_l.place(x = 480, y = 460, width = 55, height = 55)

        ang_r =  Button(key, text='>', width=6, command=lambda:  self.press('>'))
        ang_r.place(x = 540, y = 460, width = 55, height = 55)

        question =  Button(key, text='?', width=6,
                              command=lambda:  self.press('?'))
        question.place(x = 600, y = 460, width = 55, height = 55)

        clear =  Button(key, text='Clear', width=6, command=self.Clear)
        clear.place(x = 660, y = 460, width = 55, height = 55)

        num_0 =  Button(key, text='0', width=6, command=lambda:  self.press('0'))
        num_0.place(x=903, y = 460, width = 80, height = 50)
        # Fifth Line Buttons

        space =  Button(key, text='Space', width=6,
                           command=lambda:  self.press(' '))
        space.place(x = 120, y = 520, width = 450, height = 70)

       

class HomeScreen():
    def __init__(self,home=True):
        self.btn1="Container 1"
        self.btn2="Container 2"
        self.btn3="Container 3"
        self.btn4="Container 4"
        self.home=home
        self.gui()

    def gui(self):
        self.w1 = Tk()
        self.w1.geometry('1024x600')
        self.w1.maxsize(width=1024,height=600)
        self.w1.minsize(width=1024,height=600)
        self.label = Label(self.w1, text="Hello, please select a container to continue",font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 12, weight = "normal"), cursor = "arrow", state = "normal",bg="#ffffff")
        self.label.place(x = 60, y = 20, width = 390, height = 52)
        self.button1 = Button(self.w1, font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button1.place(x = 150, y = 100, width = 200, height = 52)
        self.button1['command'] = lambda: self.configure(1)
        self.button1['text'] = self.btn1
        self.button2 = Button(self.w1, font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button2.place(x = 150, y = 180, width = 200, height = 52)
        self.button2['command'] =lambda: self.configure(2)
        self.button2['text'] = self.btn2
        self.button3 = Button(self.w1,font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button3.place(x = 150, y = 260, width = 200, height = 52)
        self.button3['command'] =  lambda:self.configure(3)
        self.button3['text'] = self.btn3
        self.button4 = Button(self.w1, font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button4.place(x = 150, y = 340, width = 200, height = 52)
        self.button4['command'] = lambda:self.configure(4)
        self.button4['text'] = self.btn4
      
       
    def configure(self,param):
        global text_box_label
        self.w1.destroy()
        self.home=False
        self.w1 = Tk()
        sreenKeyboard=onKeyboard()
        self.w1.geometry('1024x600')
        self.w1.maxsize(width=1024,height=600)
        self.w1.minsize(width=1024,height=600)
        self.w1.configure(bg = '#00ffff')
        text_box_label = Entry(self.w1, font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 16), cursor = "arrow", state = "normal")
        text_box_label.place(x = 350, y = 150, width = 350, height = 42)
        self.button1 = Button(self.w1, text = "Save", font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button1.place(x = 300, y = 200, width = 120, height = 62)
        self.button1['command'] = lambda:self.Save(param)
        self.button2 = Button(self.w1, text = "Reset", font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button2.place(x = 630, y = 200, width = 130, height = 62)
        self.button2['command'] = lambda:self.Reset(param)
        self.button2 = Button(self.w1, text = "Run", font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 7), cursor = "arrow", state = "normal")
        self.button2.place(x = 480, y = 200, width = 100, height = 62)
        self.button2['command'] = lambda:self.motor_command(param)
        self.label = Label(self.w1, text="Please type the name of the pill", font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 14, weight = "normal"), cursor = "arrow", state = "normal",bg='#ffff00')
        self.label.place(x = 350, y = 80, width = 350, height = 50)
        self.button3 = Button(self.w1, text = "Home", font = tkinter.font.Font(family = "MS Shell Dlg 2", size = 8, weight = "normal"), cursor = "arrow", state = "normal")
        self.button3.place(x = 460, y = 10, width = 120, height = 62)
        self.button3['command'] = self.Home
        sreenKeyboard.text_label=text_box_label
        sreenKeyboard.display(self.w1)

    def Save(self,param):
        global text_box_label
        btn_label=str(text_box_label.get())
        btn_label=btn_label.rstrip("\n")
        if param==1:
           self.btn1=btn_label
        elif param==2:
           self.btn2=btn_label
        elif param==3:
           self.btn3=btn_label
        elif param==4:
           self.btn4=btn_label
        print('Save')

    def Reset(self,param):
        if param==1:
           self.btn1="Container 1"
        elif param==2:
           self.btn2="Container 4"
        elif param==3:
           self.btn3="Container 3"
        elif param==4:
           self.btn4="Container 4"
        print('Reset')
    def motor_command(self,param):
         if param==1:
            self.motor_1()
         elif param==2:
            self.motor_2()
         elif param==3:
            self.motor_3()
         elif param==4:
            self.motor_4()
   
    def motor_1(self):
       print("Motor 1")
       GPIO.setmode(GPIO.BOARD)
       GPIO.setup(7,GPIO.OUT)

       pwm=GPIO.PWM(7,50)
       pwm.start(0)

       pwm.ChangeDutyCycle(5) #left
       sleep(1)
       #pwm.ChangeDutyCycle(7.5) # neutral position not neeeded since its a 360deg servo
       #sleep(1)
       pwm.ChangeDutyCycle(12.5) # right
       sleep(1)

       pwm.stop()
       GPIO.cleanup()  

    def motor_2(self):
       print("Motor 2")
       GPIO.setmode(GPIO.BOARD)
       GPIO.setup(11,GPIO.OUT)

       pwm=GPIO.PWM(11,50)
       pwm.start(0)

       pwm.ChangeDutyCycle(5) #left
       sleep(1)
       pwm.ChangeDutyCycle(12.5) # right
       sleep(1)

       pwm.stop()
       GPIO.cleanup() 
    
    def motor_3(self):
        print("Motor 3")
        GPIO.setmode(GPIO.BOARD)
        GPIO.setup(13,GPIO.OUT)

        pwm=GPIO.PWM(13,50)
        pwm.start(0)

        pwm.ChangeDutyCycle(5) #left
        sleep(1)
        pwm.ChangeDutyCycle(12.5) # right
        sleep(1)

        pwm.stop()
        GPIO.cleanup()
    def motor_4(self):
        print("Motor 4")
        GPIO.setmode(GPIO.BOARD)
        GPIO.setup(15,GPIO.OUT)

        pwm=GPIO.PWM(15,50)
        pwm.start(0)

        pwm.ChangeDutyCycle(5) #left
        sleep(1)
        pwm.ChangeDutyCycle(12.5) # right
        sleep(1)

        pwm.stop()
        GPIO.cleanup()   
          
    def Home(self):
        self.w1.destroy()
        self.gui()

if __name__ == '__main__':
    a = HomeScreen(0)
    a.w1.mainloop()


# # create connection to database
#     conn = sqlite3.connect('pills.db')


# #create cursor
# c = conn.cursor()

# # create table
# c.execute('''CREATE TABLE UsersPills (
#             PillID int,
#             name text,
#             time integer
#             )''')

# #commit changes
# conn.commit()

# #close connection
# conn.close()
